...
def gradient_descent(f, df, x0, step_size, max_iter):
    ...
    for i in range(max_iter):
        xs[i] = xs[i-1] - step_size * df(xs[i-1])
        fs[i] = f(xs[i])
    return xs[-1], fs, xs
...
if __name__ == '__main__':
    ...
    x0 = x[99] # start at the last value in the array
    f1_vec = np.vectorize(f1)
    y = f1_vec(x)
    fig_simple, ax_simple = plt.subplots()  # create an empty plot and retrieve the 'ax' handle
    ax_simple.plot(x, y)
    plt.plot(x0, f1(x0), marker="o", markersize=20, markeredgecolor="red", markerfacecolor="red")
    ...
    last_x, fs, xs = gradient_descent(f1, df1, x0, 0.1, max_iter=10000)
    # Todo: Plot the found 'x' value and f(x)
    plt.plot(last_x,fs[-1], marker="o", markersize=20, markeredgecolor="blue", markerfacecolor="green")
    # Todo: Plot each step of gradient descent, to see how it converges/diverges
    for i in range(len(xs)):
        plt.plot(xs[i], fs[i], marker="o", markersize=3, markeredgecolor="black", markerfacecolor="black")
